...
def test_chrf(hypotheses, references, expected_score):
    ...
    fastchrf_scores = aggregate_chrf([hypotheses], [references], char_order=6, beta=3, eps_smoothing=True)
    assert abs(fastchrf_scores[0][0] - expected_score) < EPSILON
...
def test_chrf_eff_order(hypotheses, references, expected_score):
    ...
    fastchrf_scores = aggregate_chrf([hypotheses], [references], char_order=6, beta=3, eps_smoothing=False)
    assert abs(fastchrf_scores[0][0] - expected_score) < EPSILON
...
def test_chrf_keep_whitespace(hypotheses, references, expected_score):
    ...
    fastchrf_scores = aggregate_chrf([hypotheses], [references], char_order=6, beta=3, remove_whitespace=False)
    assert abs(fastchrf_scores[0][0] - expected_score) < EPSILON
...
def test_chrf_sentence_level(hypothesis, references, expected_score):
    ...
    fastchrf_scores = aggregate_chrf([[hypothesis]], [references], char_order=6, beta=2, eps_smoothing=True)
    assert abs(fastchrf_scores[0][0] - expected_score) < EPSILON


@pytest.mark.parametrize("hypotheses, references, expected_score", test_cases)
def test_batching(hypotheses, references, expected_score):
    fastchrf_scores = aggregate_chrf(5 * [hypotheses], 5 * [references], char_order=6, beta=3, eps_smoothing=True)
    assert abs(fastchrf_scores[0][0] - expected_score) < EPSILON
    assert abs(fastchrf_scores[1][0] - expected_score) < EPSILON
    assert abs(fastchrf_scores[2][0] - expected_score) < EPSILON
    assert abs(fastchrf_scores[3][0] - expected_score) < EPSILON
    assert abs(fastchrf_scores[4][0] - expected_score) < EPSILON
